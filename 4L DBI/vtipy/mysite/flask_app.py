...
def index():
    ...
    with mydb() as db:
        ...
        cur.execute("SELECT id, nazev, COUNT(id_kateg) FROM kateg LEFT JOIN zarazeni ON zarazeni.id_kateg=kateg.id GROUP BY id ")
        nazvy=cur.fetchall()
        try:
            if "kateg" in request.form:
                ...
                id_kateg=1
            cur.execute("SELECT nadpis, obsah, id FROM vtip,zarazeni WHERE zarazeni.id_vtip = vtip.id AND zarazeni.id_kateg = ?", (id_kateg,))
    ...
    if session.get("uziv"):
        cur.execute("SELECT COUNT(id) FROM vtip WHERE id_uziv = ?", (session.get("id_uziv"),))
    ...
    return render_template("index.html", nazvy=nazvy,vtipy=vtipy,chyba=chyba,count=count,count_uziv=count_uziv)
...
def topvtip():
    ...
    return render_template("topvtip.html", nazvy=nazvy,vtipy=vtipy,chyba=chyba,count=count,count_uziv=count_uziv)
...
def pridat_vtip():
    ...
    if request.method=="POST":
        ...
        with mydb() as db:
            cur=db.cursor()
            cur.execute("INSERT INTO vtip (nadpis,obsah,id_uziv) VALUES (?,?,?)",(nadpis,obsah,session["id_uziv"]))
            db.commit()
            cur.execute("SELECT id FROM vtip where nadpis=? and obsah=? and id_uziv=?",(nadpis,obsah,session["id_uziv"]))
            id_vtip=cur.fetchone()[0]
            cur.execute("INSERT INTO zarazeni (id_vtip,id_kateg) VALUES (?,?)",(id_vtip,0))
            for kat in nazvy:
                if f"kateg{kat[0]}" in data:
                    cur.execute("INSERT INTO zarazeni (id_vtip,id_kateg) VALUES (?,?)",(id_vtip,kat[0]))
            db.commit()
...
def edit_vtip():
    ...
    return render_template("editace.html", chyba=chyba, data=data)
...
def login():
    ...
    if request.method=="POST":
        ...
        db=mydb()
        cur=db.cursor()
        cur.execute("SELECT id,login,heslo FROM uziv WHERE login=? and heslo=?;", (jm,hs))
        pocet=cur.fetchall()
        if pocet:
            session["uziv"]=jm
            ...
            if not all((jm, hs, hs2)):
                ...
                with mydb() as db:
                    ...
                    if udaje:
                        ...
                        cur.execute("INSERT INTO uziv (login, heslo) VALUES (?, ?);", (jm, md5(jm+';;;tajnyobsah;;;'+hs)))
                        db.commit()
