...
class Kalkulacka(tk.Frame):
    ...
    def kalkulacka(self):
        ...
        self.button_sum = tk.Button(
            self, text="Součet", font=self.default_font, command=self.sum
        )
        self.button_sum.grid(row=2, column=0, sticky="we")
        self.button_mul = tk.Button(
            self, text="Součin", font=self.default_font, command=self.mul
        )
        self.button_mul.grid(row=2, column=1, sticky="we")
        self.button_sub = tk.Button(
            self, text="Rozdíl", font=self.default_font, command=self.sub
        )
        self.button_sub.grid(row=3, column=0, sticky="we")
        self.button_div = tk.Button(
            self, text="Podíl", font=self.default_font, command=self.div
        )
        self.button_div.grid(row=3, column=1, sticky="we")
